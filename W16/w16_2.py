    ...
    for j in range(len(maze[i])):
        ...
        for dir in dirs:
            ...
            match maze[i][j]:
                ...
                case 'E':
                    endCoords = ((i, j), endDir)

forwardDistances = DijkstrasAlg(graph, startCoords)
ans = forwardDistances[endCoords]

eC, _ = endCoords
sC, _ = startCoords

reverseStartDir =  ReverseDir(startDir)
reverseEndDir = ReverseDir(endDir)
reverseStartCoords = (eC, reverseEndDir)
reverseEndCoords = (sC, reverseStartDir)
reverseDistances = DijkstrasAlg(graph, reverseStartCoords)
bestPathSet = set()
for i in range(len(maze)):
    for j in range(len(maze[i])):
        if maze[i][j] == "#":
        ...
        coords = (i, j)
        for dir in dirs:
            if forwardDistances[(coords, dir)] + reverseDistances[(coords, ReverseDir(dir))] == ans:
                bestPathSet.add(coords)

print(len(bestPathSet))
